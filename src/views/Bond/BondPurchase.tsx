...
function BondPurchase({ bond, slippage, recipientAddress }: IBondPurchaseProps) {
    ...
    const [useTELO, setUseTELO] = useState(false);
    ...
    const setMax = () => {
        let amount: any = Math.min(bond.maxBondPriceToken * 0.9999, useTELO ? bond.TELOBalance * 0.99 : bond.balance);
            ...
            <Box display="flex" justifyContent="space-around" flexWrap="wrap">
                {bond.name === "wTELO" && (
                    <FormControl className="ohm-input" variant="outlined" color="primary" fullWidth>
                        <div className="TELO-checkbox">
                            <input type="checkbox" checked={useTELO} onClick={() => setUseTELO(!useTELO)} />
                            <p>Use TELO</p>
                        </div>
                    </FormControl>
                )}
            ...
            <Slide direction="left" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
                <Box className="bond-data">
                    <div className="data-row">
                        ...
                        <p className="bond-balance-title">
                            {isBondLoading ? (
                                ...
                                <>
                                    {trim(useTELO ? bond.TELOBalance : bond.balance, 4)} {displayUnits}
                                </>
                            )}
                        </p>
                    </div>
                    ...
                    <div className="data-row">
                        ...
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.bondQuote, 4)} TELESTO`}</p>
                    </div>

                    <div className={`data-row`}>
                        <p className="bond-balance-title">Max You Can Buy</p>
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.maxBondPrice, 4)} TELESTO`}</p>
                    </div>
                    ...
                    <div className="data-row">
                        ...
                        <p className="bond-balance-title">0.01 TELESTO</p>
                    </div>
