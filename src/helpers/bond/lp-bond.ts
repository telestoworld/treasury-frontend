...
export class LPBond extends Bond {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isTelesto = token1.toLowerCase() === addresses.TELESTO_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isTelesto ? reserve0 : reserve1) : this.toTokenDecimal(true, isTelesto ? reserve1 : reserve0);
    }

    private toTokenDecimal(isTelesto: boolean, reserve: number) {
        return isTelesto ? reserve / Math.pow(10, 9) : reserve / Math.pow(10, 18);
    }
}
